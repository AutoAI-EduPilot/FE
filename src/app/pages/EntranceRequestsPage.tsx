...
export function EntranceRequestsPage() {
  ...
  return <PageContainer>
    ...
    {isLoading ? <p className="py-16 text-center text-sm text-stone-500" role="status">입장 정보를 불러오는 중입니다.</p> : null}
    ...
    {!isLoading && !error ? <section className="overflow-hidden rounded-lg border border-stone-200 bg-white"><div className="hidden grid-cols-[1fr_1.4fr_1fr_1fr_150px] border-b border-stone-200 bg-stone-50 px-4 py-3 text-[11px] font-semibold text-stone-400 lg:grid"><span>학생</span><span>이메일</span><span>학교·소속</span><span>{tab === 'students' ? '승인 시각' : '요청 시각'}</span><span className="text-right">상태</span></div>{requests.length === 0 ? <div className="flex min-h-64 flex-col items-center justify-center px-6 text-center">{tab === 'students' ? <Users className="text-stone-300" size={20} /> : <Inbox className="text-stone-300" size={20} />}<h2 className="mt-4 font-bold">{emptyTitle}</h2></div> : requests.map((request) => <div className="grid gap-2 border-b border-stone-100 px-4 py-3 text-sm last:border-0 lg:grid-cols-[1fr_1.4fr_1fr_1fr_150px] lg:items-center" key={request.requestId}><strong>{request.learner?.name ?? '-'}</strong><span className="text-stone-500">{request.learner?.email ?? '-'}</span><span className="text-stone-500">{request.learner?.affiliation ?? '-'}</span><span className="text-xs text-stone-400">{new Date(tab === 'students' && request.processedAt ? request.processedAt : request.requestedAt).toLocaleString('ko-KR')}</span><div className="flex justify-end gap-2">{request.status === 'PENDING' ? <><Button onClick={() => void process(request, 'reject')} size="sm" variant="ghost">거절</Button><Button onClick={() => void process(request, 'approve')} size="sm">승인</Button></> : <span className="text-xs font-semibold text-stone-500">{tab === 'students' ? '수강 중' : request.status === 'APPROVED' ? '승인됨' : '거절됨'}</span>}</div></div>)}</section> : null}
    <p className="flex w-fit items-start gap-2 rounded-lg bg-stone-50 px-3.5 py-2.5 text-xs leading-5 text-stone-500"><Info className="mt-0.5 shrink-0" size={14} />{tab === 'students' ? '현재 목록은 승인 이력을 기준으로 표시됩니다. 정확한 재학 목록과 내보내기는 백엔드 수강생 관리 API가 필요합니다.' : '초대 코드로 참여를 요청한 학습자는 강의자 승인 후 강의실에 입장합니다.'}</p>
  </PageContainer>
}

function TabButton({ active, label, onClick }: { active: boolean; label: string; onClick: () => void }) { return <button aria-selected={active} className={cx('h-8 rounded-md px-3 text-xs font-semibold', active ? 'bg-stone-900 text-white' : 'text-stone-500 hover:bg-stone-100')} onClick={onClick} role="tab" type="button">{label}</button> }
