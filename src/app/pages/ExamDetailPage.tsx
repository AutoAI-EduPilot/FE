...
function LearnerExamView({ exam, onResultReady, repository }: { exam: Exam; onResultReady: () => void; repository: ReturnType<typeof createExamsRepository> }) {
  ...
  async function submit(event: FormEvent) { event.preventDefault(); if (isSubmitting || !exam.submittable || attemptStartStatus !== 'ready') return; setIsSubmitting(true); try { const nextSubmission = await repository.submit(exam.id, answers, createRequestId()); if (draftStorageKey) removeExamDraft(draftStorageKey); setSubmission(nextSubmission); setError(null) } catch (requestError) { setError(getRequestErrorMessage(requestError)) } finally { setIsSubmitting(false) } }
...
function formatCorrectAnswer(question: ExamQuestion | undefined, answer: string | undefined): string | null {
