...
import { Badge, Button, EmptyState, PageContainer, PageHeader, useToast } from '../../shared/ui'
import { examDetailPath } from '../routes'
...
export function ExamsPage() {
  ...
  useEffect(() => {
    ...
    classroomsRepository.list('', controller.signal).then((items) => { setClassrooms(items); setClassroomId((current) => current || items[0]?.id || ''); if (items.length === 0) setIsLoading(false) }).catch((requestError) => { if (!controller.signal.aborted) { setError(getRequestErrorMessage(requestError)); setIsLoading(false) } })
  ...
  return <PageContainer>
    <PageHeader title="시험" titleAccessory={<select aria-label="강의실 선택" className="h-9 min-w-40 rounded-lg border border-stone-200 bg-white px-3 type-control font-semibold" onChange={(event) => { setIsLoading(true); setClassroomId(event.target.value) }} value={classroomId}>{classrooms.length === 0 ? <option value="">강의실 없음</option> : classrooms.map((classroom) => <option key={classroom.id} value={classroom.id}>{classroom.name}</option>)}</select>} actions={isInstructor ? <Button disabled={!classroomId} onClick={() => setIsComposerOpen(true)}><Plus size={15} />시험 만들기</Button> : undefined} />
