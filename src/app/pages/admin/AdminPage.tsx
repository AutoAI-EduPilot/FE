        ...
        <div className="overflow-x-auto pb-1">
          <div className="flex min-w-max items-end justify-around gap-5" role="img" aria-label="날짜별 AI 성공 및 실패 호출 막대 차트">
            {daily.map((day) => {
              ...
              return <div className="flex w-16 shrink-0 flex-col items-center" key={day.date} title={`${day.date}: 성공 ${formatCount(day.successCount)}건, 실패 ${formatCount(day.failCount)}건, 토큰 ${formatCount(tokens)}`}><strong className="mb-1 type-caption text-stone-700">{formatCount(day.callCount)}</strong><div className="flex h-28 w-7 flex-col justify-end overflow-hidden rounded-t-sm"><span className="w-full bg-amber-500" style={{ height: `${failHeight}%` }} /><span className="w-full bg-brand-700" style={{ height: `${Math.max(day.successCount > 0 ? 3 : 0, successHeight)}%` }} /></div><span className="mt-2 type-caption text-stone-500">{formatMonthDay(day.date)}</span><span className="type-micro text-stone-400">{formatCompactNumber(tokens)}</span></div>
            })}
          </div>
        </div>
      )}
...
function FeatureUsageChart({ summary }: { summary: AiUsageSummary | null }) {
  ...
  return <section className="px-4 py-4" aria-labelledby="feature-usage-title"><h3 className="mb-4 type-control font-bold text-stone-900" id="feature-usage-title">기능별 호출</h3>{features.length === 0 ? <p className="py-10 text-center type-body text-stone-500">선택한 기간의 기능별 기록이 없습니다.</p> : <div className="overflow-x-auto pb-1"><div className="flex min-w-max items-end justify-around gap-5" role="img" aria-label="기능별 AI 호출 막대 차트">{features.map((item) => <div className="flex w-20 shrink-0 flex-col items-center" key={item.feature} title={`${featureLabel(item.feature)}: ${formatCount(item.callCount)}건, 토큰 ${formatCount(tokenTotal(item))}`}><strong className="mb-1 type-caption text-stone-700">{formatCount(item.callCount)}</strong><span className="w-7 rounded-t-sm bg-brand-700" style={{ height: `${Math.max(3, (item.callCount / maxCalls) * 110)}px` }} /><span className="mt-2 max-w-full truncate type-caption text-stone-600">{featureLabel(item.feature)}</span><span className="type-micro text-stone-400">{formatCompactNumber(tokenTotal(item))}</span></div>)}</div></div>}</section>
}
