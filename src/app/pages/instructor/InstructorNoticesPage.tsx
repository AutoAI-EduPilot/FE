...
export function InstructorNoticesPage() {
  ...
  return <PageContainer>
    ...
    {isLoading ? <p className="py-16 text-center text-sm text-stone-500" role="status">공지를 불러오는 중입니다.</p> : null}
    ...
    {notices.length > 0 ? <section className="overflow-hidden rounded-lg border border-stone-200 bg-white" aria-label="공지 목록">{notices.map((notice) => <article className="border-b border-stone-100 px-5 py-4 last:border-0" key={notice.id}><div className="flex items-center gap-3"><BellRing className="text-brand-600" size={15} /><h2 className="font-bold text-stone-900">{notice.title}</h2><time className="ml-auto text-xs text-stone-400">{new Date(notice.publishedAt).toLocaleDateString('ko-KR')}</time><Button aria-label={`${notice.title} 수정`} onClick={() => setEditingNotice(notice)} size="sm" variant="secondary"><Pencil aria-hidden="true" size={13} />수정</Button><Button aria-label={`${notice.title} 내리기`} disabled={removingId === notice.id} onClick={() => void removeNotice(notice)} size="sm" variant="ghost"><Trash2 aria-hidden="true" size={13} />내리기</Button></div><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-stone-600">{notice.content}</p></article>)}</section> : null}
