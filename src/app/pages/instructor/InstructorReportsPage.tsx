...
import { createClassroomsRepository, rememberClassroomId, type ClassroomWeek } from '../../../features/classrooms'
import {
...
export function InstructorReportsPage() {
  ...
  return <PageContainer>
    <PageHeader
      actions={<ButtonLink to={classroomReportCriteriaPath(classroomId)} variant="secondary"><Settings2 size={14} />평가 기준</ButtonLink>}
      title="학습 리포트"
    />
  ...
  return <PageContainer>
    ...
    {reportsEnabled && !isLoading ? <>
      <section className="rounded-lg border border-stone-200 bg-white p-5" aria-labelledby="report-scope-title">
        ...
        {scopeType === 'WEEK' ? <div className="mt-4 flex flex-wrap gap-2" role="radiogroup" aria-label="분석 주차">{weeks.map((week) => <label className={`inline-flex min-h-9 cursor-pointer items-center gap-2 rounded-lg border px-3 type-control ${selectedWeek === week.weekNumber ? 'border-brand-600 bg-brand-50 text-brand-800' : 'border-stone-200 text-stone-600'}`} key={week.weekNumber}><input checked={selectedWeek === week.weekNumber} className="sr-only" name="report-week" onChange={() => setSelectedWeek(week.weekNumber)} type="radio" />{week.weekNumber}주차 · {week.title}</label>)}</div> : null}
      </section>
