  ...
  return (
    <div>
      ...
      >
        <label
          className={
            labelHidden
              ? 'sr-only'
              : 'shrink-0 text-sm font-semibold text-stone-800'
          }
          htmlFor={id}
        >
          {label}
        ...
        {error && !labelHidden ? (
          <p
            className="min-w-0 text-right text-xs font-medium leading-tight text-rose-700"
            id={`${id}-error`}
            role="alert"
          >
            {error}
      ...
      <textarea
        ...
        className={cx(
          'mt-1.5 block min-h-20 w-full resize-y rounded-lg border bg-white px-3 py-2 text-sm text-stone-950',
      ...
      />
      {description ? (
        <p className="mt-1 text-xs text-stone-500" id={`${id}-description`}>
          {description}
        </p>
      ) : null}
      {error && labelHidden ? (
